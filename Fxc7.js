...
prefix = "!"
name = "~ ANBU BOT"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN ANBU_BOT😁"
rmenu = "HAI TEMEN ANBU_BOT👋 JANGAN LUPA DONASI YAA:)"
botinfo = "UNTUK INVITE BOT SILAHKAN DONASI DULU YAA:)"
limitt = 20
memberLimit = 2
ban = []
premium = ["6285820629269@s.whatsapp.net"]
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} ANBU_BOT👋 MISS YOU:D`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6285820629269@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
					...
					if (isUser) return reply('Kamu sudah Menjadi Temen ANBU_BOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat ANBU_BOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *ANBU_BOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ANBU_BOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
